...
class MarketplaceWebService_Client implements MarketplaceWebService_Interface
{
   ...
   * @throws MarketplaceWebService_Exception
   */
  public function submitFeed($request, $feedFileName = "")
  {
    ...
    return MarketplaceWebService_Model_SubmitFeedResponse::fromXML(
    	$this->invoke($this->convertSubmitFeed($request), $request->getFeedContent(), $request->getContentMd5(), $feedFileName));
  }
   ...
   */
  private function invoke(array $converted, $dataHandle = null, $contentMd5 = null, $feedFileName = "")
  {
    ...
    if (empty($this->config['ServiceURL'])) {
      ...
      do {
        try {
          ...
          $response = $this->performRequest($actionName, $converted, $dataHandle, $contentMd5, $feedFileName);
  ...
  private function reportAnyErrors($responseBody, $status, Exception $e =  null)
  {
    ...
    if (!is_null($responseBody) && strpos($responseBody, '<') === 0) {
      ...
      $responseBody, $errorMatcherThree)) {
        ...
        require_once ('Exception.php');
        $ex = new MarketplaceWebService_Exception(array('Message' => 'Internal Error', 'StatusCode' => $status, 'XML' => $responseBody));
      }
    } else {
      require_once ('Exception.php');
      $ex = new MarketplaceWebService_Exception(array('Message' => 'Internal Error', 'StatusCode' => $status, 'XML' => $responseBody));
   ...
   * @return array
   */
  private function performRequest($action, array $converted, $dataHandle = null, $contentMd5 = null, $feedFileName = "") {
  	

    $curlOptions = $this->configureCurlOptions($action, $converted, $dataHandle, $contentMd5, $feedFileName);
   ...
   * @return array
   */
  private function configureCurlOptions($action, array $parameters, $streamHandle = null, $contentMd5 = null, $feedFileName = "") {
    ...
    if ($requestType == RequestType::POST_UPLOAD) {
      ...
      if($feedFileName == "") {
      	...
      	$fileChunk = file_get_contents($feedFileName);
      	$curlOptions[CURLOPT_INFILE] = $fileChunk;
      	$curlOptions[CURLOPT_INFILESIZE] = strlen($fileChunk);
      }
...
//print_r($curlOptions); die;
  ...
  private function getDownloadResponseDocument($responseType, $header) {
    ...
    $response = stream_get_contents($this->responseBodyContents);
    return $response;
  }
