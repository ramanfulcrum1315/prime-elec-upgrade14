...
class Camiloo_Amazonimport_Block_Selection_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        $this->_updateButton('save', 'label', Mage::helper('amazonimport')->__('Save Settings'));
    ...
    public function getHeaderText()
    {
        if( Mage::registry('amazonimport_data') && Mage::registry('amazonimport_data')->getId() ) {
            return Mage::helper('amazonimport')->__("Select to list this product on Amazon '%s'", $this->htmlEscape(Mage::registry('amazonimport_data')->getTitle()));
        } else {
            return Mage::helper('amazonimport')->__('Select to list this product on Amazon');
        }
    }
}