...
//	const EXPR_SHORTCODE_PARAM = '[ ]{1}([^ ]{1,})=["]{0,1}(.*)["]{0,1}[ \]]{1}';
	...
	{	
		...
		// Convert raw YouTube URL's to shortcodes
		if (preg_match_all('/[^"=]{2,}(http:\/\/www.youtube.com\/watch\?[a-z0-9\&=_\-]{1,})/i', $content, $result)) {
			foreach($result[1] as $url) {
				$content = str_replace($url, sprintf('[youtube="%s"]', $url), $content);
			}
		}
	...
	{
		if (strpos($content, '[associated-products') === false) {
			return $this;
		}
		...
		if (($shortcodes =	$this->_getShortcodesByTag($content, 'gallery')) !== false) {
			foreach($shortcodes as $it => $shortcode) {
				...
				if (($ids = trim($args->getIds(), ',')) !== '') {
					...
					$images = $params['object']->getImages();
				}
